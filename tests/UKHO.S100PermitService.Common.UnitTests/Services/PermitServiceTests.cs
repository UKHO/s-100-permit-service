...
namespace UKHO.S100PermitService.Common.UnitTests.Services
{
    ...
    public class PermitServiceTests
    {
        ...
        [Test]
        public async Task WhenPermitRequestHasValidProductsAndUpns_PermitServiceReturnsSuccessResponse()
        {  
            var permitRequest = new PermitRequest();

            var result = await _permitService.ProcessPermitRequestAsync(PRODUCT_TYPE, permitRequest, _fakeCorrelationId, CancellationToken.None);
        ...
        //                userPermitServiceResponse.UserPermits = [new Models.UserPermitService.UserPermit { Title = "Title", Upn = "Upn" }];
        //            }
