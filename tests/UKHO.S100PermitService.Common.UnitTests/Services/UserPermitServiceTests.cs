...
namespace UKHO.S100PermitService.Common.UnitTests.Services
{
    ...
    public class UserPermitServiceTests
    {
        ...
        public async Task WhenLicenceIdNotFound_ThenUserPermitServiceReturnsNotFoundResponse()
        {
            ...
            A.CallTo(_fakeLogger).Where(call =>
                ...
                call.Method.Name == "Log"
                    ...
                    && call.GetArgument<IEnumerable<KeyValuePair<string, object>>>(2).ToDictionary(c => c.Key, c => c.Value)["{OriginalFormat}"].ToString() == "Request to UserPermitService GET {RequestUri} failed. StatusCode: {StatusCode} | Error Details: {Errors}"
                ).MustHaveHappenedOnceExactly();
        }
        ...
        public async Task WhenLicenceIdIs0_ThenUserPermitServiceReturnsException400WithErrorDetails(int licenceId, HttpStatusCode statusCode, string content)
        {
            ...
            await FluentActions.Invoking(async () => await _userPermitService.GetUserPermitAsync(licenceId, CancellationToken.None, _fakeCorrelationId)).Should().ThrowAsync<PermitServiceException>().WithMessage("Request to UserPermitService GET {RequestUri} failed. StatusCode: {StatusCode} | Error Details: {Errors}");
