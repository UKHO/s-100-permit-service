    ...
    public class S100CryptTests
    {
        ...
        public void WhenProductKeysDecryptedSuccessfully_ThenReturnsDecryptedKeys()
        {
            ...
            A.CallTo(_fakeLogger).Where(call =>
                ...
                && call.GetArgument<IEnumerable<KeyValuePair<string, object>>>(2)!.ToDictionary(c => c.Key, c => c.Value)["{OriginalFormat}"].ToString() == "Get Encrypted keys from Product keys completed."
            ).MustHaveHappened();
        }
